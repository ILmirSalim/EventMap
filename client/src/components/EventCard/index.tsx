...
export const EventCard = () => {
  ...
  const addUserToEvent = async (eventId: string, userId: string | undefined, userName: string
  ) => {
    try {
      setIsLoading(true);
  ...
  return (
    ...
    flex flex-col justify-center items-center rounded-xl">
      <div className="text-center mt-[-50px] border-white border p-[10px] w-full rounded-xl">
        ...
        <p><span className="font-bold">Описание события:</span> {event.description}</p>
        <p><span className="font-bold">Местоположение:</span> {event.locationType}</p>
        <p><span className="font-bold">Адрес:</span> {event.address}</p>
        <p><span className="font-bold">Рейтинг мероприятия:</span></p>
        <p>{resultRate && resultRate}</p>
        <Link to={`/updateEvent/${event._id}`} className="p-[10px] rounded-xl 
        mt-[10px] cursor-pointer hover:text-white">
      ...
      <div className="flex flex-col items-center text-center border-white mt-[50px] 
